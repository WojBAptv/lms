...
import { projectCreateSchema, projectSchema, type Project, posInt } from "../types.js";

const router = Router();
const DATA_FILE = path.resolve(process.cwd(), "data", "projects.json");
(async () => { await ensureDataFile(DATA_FILE, [] as Project[]); })();

async function getAll(): Promise<Project[]> {
  const items = await readJson<Project[]>(DATA_FILE);
  return items.sort((a, b) => a.id - b.id);
}
async function saveAll(items: Project[]) { await writeJson(DATA_FILE, items); }
...
router.post("/", async (req, res) => {
  ...
  const next: Project = { id, programId: parsed.data.programId, name: parsed.data.name };
  items.push(next);
  await saveAll(items);
